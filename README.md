...
- Для waitlist-уведомления (персональная доставка пользователю) рассылается:

```json
{"type":"waitlist_slot_available","roomId":"...","userId":"...","slotId":"...","waitlistEntryId":"...","timestamp":"..."}
```
...
- При `POST /bookings/{bookingId}/cancel` (только когда слот реально освобождён) сервис atomically выбирает первый `active` entry (`FOR UPDATE SKIP LOCKED`), переводит его в `notified` и публикует персональный realtime event `waitlist_slot_available`.
- V1 ограничения: **без auto-reserve**, **без TTL/expiration worker**, **без retries/outbox**.
...
| 404 | `ROOM_NOT_FOUND`, `SLOT_NOT_FOUND`, `BOOKING_NOT_FOUND`, `WAITLIST_NOT_FOUND` | сущность не найдена |
| 409 | `SLOT_ALREADY_BOOKED`, `SLOT_NOT_BOOKED`, `SCHEDULE_EXISTS`, `WAITLIST_ALREADY_JOINED` | конфликт брони/очереди/расписания |
...
- `internal/usecase` — сценарии (room, schedule, slot, booking, waitlist, auth).
